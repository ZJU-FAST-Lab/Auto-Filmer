...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  ros::Publisher gridmap_pub_, gridmap_inflate_pub_, local_pc_pub_, pcl_pub_, pc_image_pub_;

  // NOTE just for global map in simulation
  ros::Timer global_map_timer_;
  ros::Subscriber map_pc_sub_, nv_image_sub_;
  ...
  void nv_image_callback(const sensor_msgs::ImageConstPtr& msg)
  {
    pc_image_pub_.publish(msg);
  }
  ...
  void init(ros::NodeHandle& nh) {
    ...
    // zzw 0314
    nv_image_sub_ = nh.subscribe<sensor_msgs::Image>("nv_image", 1, &Nodelet::nv_image_callback, this, ros::TransportHints().tcpNoDelay());
    pc_image_pub_ = nh.advertise<sensor_msgs::Image>("pc_image", 10);
  }
