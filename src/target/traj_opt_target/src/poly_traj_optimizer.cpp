...
namespace ego_planner
{
      ...
      Eigen::MatrixXd &optimal_points, double &final_cost)
  {
    ...
    bool flag_force_return, flag_still_occ, flag_success, flag_swarm_too_close;
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        flag_swarm_too_close = min_ellip_dist2_ < pow((swarm_clearance_ * 1.25), 2);
        if (!flag_swarm_too_close)
        {
          if (finelyCheckAndSetConstraintPoints(segments_nouse, jerkOpt_, false) == CHK_RET::OBS_FREE)
          {
            ...
            // printf( VERBOSE_OUTPUT ? "\033[32miter=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost=%5.3f\n\033[0m":"", iter_num_, time_ms, total_time_ms, final_cost);
          }
          else
          {
            ...
            // printf(VERBOSE_OUTPUT ? "\033[32miter=%d,time(ms)=%5.3f, fine check collided, keep optimizing\n\033[0m":"", iter_num_, time_ms);
          }
        }
        else
        {
          PRINTF_COND("swarm are too close, return and wait for next planning. Min dist=%f, allowed min dist=%f\n", sqrt(min_ellip_dist2_), swarm_clearance_ * 1.25);
          // printf(VERBOSE_OUTPUT ? "swarm are too close, return and wait for next planning. Min dist=%f, allowed min dist=%f\n":"", sqrt(min_ellip_dist2_), swarm_clearance_ * 1.25);
        }
      }
