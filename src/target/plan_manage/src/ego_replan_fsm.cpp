...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    {
      trigger_sub_ = nh.subscribe("/move_base_simple/goal", 1, &EGOReplanFSM::triggerCallback, this);
  ...
  find_ij_start:;
    ...
    for (size_t i = i_start; i < i_end; ++i)
    {
      for (size_t j = j_start; j < pts_chk[i].size(); ++j)
      {
        ...
        dangerous |= map->getLessInflateOccupancy(p);
  ...
  void EGOReplanFSM::waypointCallback(const geometry_msgs::PoseStampedConstPtr &msg)
  {
    ...
    ROS_INFO("Received goal: %f, %f, %f", msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
