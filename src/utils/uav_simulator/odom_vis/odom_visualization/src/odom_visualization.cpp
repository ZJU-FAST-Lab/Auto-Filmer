...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  meshROS.header.stamp = ros::Time::now(); //msg->header.stamp;
  meshROS.ns = "mesh";
  meshROS.id = 0;
  ...
  poseROS.header.stamp = ros::Time::now(); // msg->header.stamp;
  ...
  velROS.header.stamp = ros::Time::now(); // msg->header.stamp;
  ...
  if (1) {
    ...
    broadcaster->sendTransform(tf::StampedTransform(transform, stamp, string("world"), string("/base")));
    broadcaster->sendTransform(tf::StampedTransform(transform45, stamp, string("/base"), string("/laser")));
    broadcaster->sendTransform(tf::StampedTransform(transform45, stamp, string("/base"), string("/vision")));
    broadcaster->sendTransform(tf::StampedTransform(transform90, stamp, string("/base"), string("/height")));
  }
}
